...
records = [
    ...
    # ── 批量异常1：银饰品材质不符 ──
    ["B1-01", "买的银手镯说是999纯银，拿回来一测根本不是，含银量最多60%，这算不算欺诈", 399, "2026-05-15"],
    ["B1-02", "这个银项链掉色也太严重了吧，戴了两天脖子都绿了，根本不是纯银的", 259, "2026-05-15"],
    ["B1-03", "S925银戒指，结果戴了一周就发黑，我以前买的银饰戴一年都不会这样，肯定是假的", 329, "2026-05-15"],
    ["B1-04", "银耳钉收到就有铜锈味，这是银的吗？我要退货退款", 189, "2026-05-16"],
    ["B1-05", "买的银饰套盒，里面好几件都有氧化斑点，商家非说是正常现象，这不是忽悠人吗", 499, "2026-05-16"],
    ["B1-06", "这个银镯子上面明明写的S925，但检测出来是铜镀银，假冒伪劣！我要去315举报", 599, "2026-05-16"],
    ["B1-07", "银项链的材质跟详情页完全不符，证书也是假的，太坑人了", 429, "2026-05-16"],
    ["B1-08", "买了两对银耳环都掉色，而且掉色之后里面露出来的是红色的，这根本就是铜的", 239, "2026-05-17"],
    ["B1-09", "那个银饰商家太黑了，我买的银手镯材质完全不对，而且多个买家都反映有这个问题", 369, "2026-05-17"],
    ["B1-10", "又是银饰，又是材质不符，你们平台到底管不管这类商家", 279, "2026-05-17"],
    ["B1-11", "S925银饰套链，收到货根本不是银的，戴了一次就过敏起疹子，要求退款加赔偿", 549, "2026-05-17"],
    ["B1-12", "银饰手镯所谓的\"纯银\"承诺完全是虚假宣传，材质检测根本不过关，建议彻查该类商家", 699, "2026-05-17"],

    # ── 批量异常2：台湾集运物流积压 ──
    ["B2-01", "台湾集运的包裹已经等了一个月了还没到，物流显示一直在中转", 399, "2026-05-16"],
    ["B2-02", "集运包裹卡在中转站不动了，客服也联系不上，我的货到底在哪", 459, "2026-05-16"],
    ["B2-03", "台湾流向的物流是不是出了什么问题，我的集运订单已经有20多天没更新物流了", 329, "2026-05-17"],
    ["B2-04", "三个集运包裹全部积压，问了物流公司说是运力不够", 589, "2026-05-17"],
    ["B2-05", "集运台湾的订单物流已经超过30天，打了好多次电话都说在处理", 429, "2026-05-17"],
    ["B2-06", "我的集运包裹显示异常，问客服说是在协调，但是等了一周没任何进展", 359, "2026-05-18"],
    ["B2-07", "台湾集运商到底什么时候能恢复，我的订单各种节日礼物等着用呢", 529, "2026-05-18"],
    ["B2-08", "物流积压这么严重，你们平台至少应该主动通知消费者", 289, "2026-05-18"],
...
df.to_csv("sample_complaints.csv", index=False, encoding="utf-8-sig")
print(f"已生成 sample_complaints.csv，共 {len(df)} 条记录")
print(f"埋点：银饰品批量异常12条（5/15-5/17）+ 台湾集运批量异常8条（5/16-5/18）")
